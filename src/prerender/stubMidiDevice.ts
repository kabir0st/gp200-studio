...
/**
 * A frozen, permanently-disconnected device for the prerendered front pages.
 *
 * `StartScreen` and `Landing` read six things off `useMidiDevice()` — status,
 * handshakeStep, errorMessage, currentSlot, presetNames and connect — and never
 * call the other fifty-odd senders during render. Rather than stub every method (fifty
 ...
 * The moment the real app boots, createRoot replaces this markup with the
 * same page wired to the live hook.
 */
const DISCONNECTED = {
  status: 'disconnected',
  handshakeStep: null,
